					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/ticket_reservation/edit','add')" @click="to_form('/ticket_reservation/edit')" >预定 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/scenic_spot_information/details','set')" @click="change_praise()" >点赞</button>
...
<script>
	...
	export default {
          ...
          if (res.result || res.result === 0) {
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.img = this.obj.scenic_spot_cover
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        varscenic_spot_information_id = this.obj.scenic_spot_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "scenic_spot_information",
          source_field: "scenic_spot_information_id",
          source_id:scenic_spot_information_id
        }, (res) => {
