					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/hotel_reservation/edit','add')" @click="to_form('/hotel_reservation/edit')" >预定 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/hotel_information/details','set')" @click="change_praise()" >点赞</button>
...
<script>
	...
	export default {
          ...
          if (res.result || res.result === 0) {
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.img = this.obj.hotel_cover
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        varhotel_information_id = this.obj.hotel_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "hotel_information",
          source_field: "hotel_information_id",
          source_id:hotel_information_id
        }, (res) => {
