						...
						<div class="card">
							...
							<div class="form_register">
								<b-form>
									...
									<!--景点信息-->
									<form_scenic_spot_information v-if="form.user_group === '景点'" :form="form_sub"></form_scenic_spot_information>
...
<script>
	...
	/* 景点信息 */
	import form_scenic_spot_information from "@/components/diy/form_scenic_spot_information";
		...
		methods: {
			...
			async submit_check(param){
				...
				if(!ret && user_group){
					...
					if(user_group === "景点"){
						var form_sub = Object.assign({} ,this.form_sub);
						if(!ret && !form_sub["attraction_number"]){
							ret = "景点编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/scenic_spot_information/count?", {"attraction_number": form_sub["attraction_number"]});
							if(res.result){
								ret = "景点编号已存在!";
							}
						}
